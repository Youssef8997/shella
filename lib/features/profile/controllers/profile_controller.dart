...
class ProfileController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> updateUserInfo(UserInfoModel updateUserModel, String token) async {
    _isLoading = true;
    update();
    log(updateUserModel.toJson().toString());
...
List<Item>item=[];
Future getItemStatic()async{
  item=[];

  Response response=await profileServiceInterface.getStaticItem();
  if(response.statusCode==200){
    item=List.generate(response.body.length, (value){
      return Item.fromJson(response.body[value]);
    });
  }
  update();
}
