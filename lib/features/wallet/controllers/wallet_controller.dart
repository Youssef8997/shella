...
class WalletController extends GetxController implements GetxService {
  ...
  Future<void> addFundToWallet(double amount, String paymentMethod) async {
    ...
    if (response.statusCode == 200) {
      ...
      showCustomSnackBar(response.body["errors"]["message"]);
    }
    _isLoading = false;
    update();
  }
...
Future<bool> requestExchange(number)async{
    Response response =await walletServiceInterface.requestExchange(number);
   return response.statusCode==200;
}
Future<bool> Exchange(number,otp,userOtp,money)async{
    Response response =await walletServiceInterface.exchangeWallet(number,otp,userOtp,money);

    if(response.statusCode!=200){

      showCustomSnackBar("تاكد من ال otp الخاص بك و otp الخاص بالحساب الاخر وتوافر المبلغ المحول معك");
