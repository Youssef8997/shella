...
class FilterWidgetCategory extends StatefulWidget {
  final double? maxValue;
  final bool isStore;
  const FilterWidgetCategory({super.key, required this.maxValue, required this.isStore});
...
class _FilterWidgetCategoryState extends State<FilterWidgetCategory> {
  @override
  void initState() {
  Get.find<search.SearchController>().setAllItemList(Get.find<CategoryController>().categoryItemList);
        ...
        child: GetBuilder<search.SearchController>(builder: (searchController) {
          return SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                itemBuilder: (context, index) {
                  return InkWell(
                    ...
                    child: Container(
                      alignment: Alignment.center,
                      decoration: BoxDecoration(
                        border: Border.all(color: (widget.isStore ? searchController.storeSortIndex == index : searchController.sortIndex == index) ? Colors.transparent
                            : Theme.of(context).disabledColor),
                        borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
                        color: (widget.isStore ? searchController.storeSortIndex == index : searchController.sortIndex == index) ? Theme.of(context).primaryColor
                            : Theme.of(context).disabledColor.withOpacity(0.1),
                      ),
                      child: Text(
                        searchController.sortList[index],
                        textAlign: TextAlign.center,
                        style: robotoMedium.copyWith(
                          color: (widget.isStore ? searchController.storeSortIndex == index : searchController.sortIndex == index) ? Colors.white : Theme.of(context).hintColor,
                        ),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
              ...
              && Get.find<SplashController>().configModel!.moduleConfig!.module!.vegNonVeg!) ? CustomCheckBoxWidget(
                title: 'veg'.tr,
                value: widget.isStore ? searchController.storeVeg : searchController.veg,
                onClick: () => widget.isStore ? searchController.toggleStoreVeg() : searchController.toggleVeg(),
              ) : const SizedBox(),

              (Get.find<SplashController>().configModel!.toggleVegNonVeg!
              && Get.find<SplashController>().configModel!.moduleConfig!.module!.vegNonVeg!) ? CustomCheckBoxWidget(
                title: 'non_veg'.tr,
                value: widget.isStore ? searchController.storeNonVeg : searchController.nonVeg,
                onClick: () => widget.isStore ? searchController.toggleStoreNonVeg() : searchController.toggleNonVeg(),
              ) : const SizedBox(),

              CustomCheckBoxWidget(
                title: widget.isStore ? Get.find<SplashController>().configModel!.moduleConfig!.module!.showRestaurantText!
                    ? 'currently_opened_restaurants'.tr : 'currently_opened_stores'.tr : 'currently_available_items'.tr,
                value: widget.isStore ? searchController.isAvailableStore : searchController.isAvailableItems,
                onClick: () {
                  if(widget.isStore) {
                    searchController.toggleAvailableStore();
                  } else {
              ...
              CustomCheckBoxWidget(
                title: widget.isStore ? Get.find<SplashController>().configModel!.moduleConfig!.module!.showRestaurantText!
                    ? 'discounted_restaurants'.tr : 'discounted_stores'.tr : 'discounted_items'.tr,
                value: widget.isStore ? searchController.isDiscountedStore : searchController.isDiscountedItems,
                onClick: () {
                  if(widget.isStore) {
                    searchController.toggleDiscountedStore();
                  } else {
              ...
              widget.isStore ? const SizedBox() : Column(children: [
                Text('price'.tr, style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge)),
                RangeSlider(
                  values: RangeValues(searchController.lowerValue, searchController.upperValue),
              ...
              Container(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return InkWell(
                      onTap: () => widget.isStore ? searchController.setStoreRating(index + 1) : searchController.setRating(index + 1),
                      child: Icon(
                        (widget.isStore ? searchController.storeRating < (index + 1) : searchController.rating < (index + 1)) ? Icons.star_border : Icons.star,
                        size: 30,
                        color: (widget.isStore ? searchController.storeRating < (index + 1) : searchController.rating < (index + 1)) ? Theme.of(context).disabledColor
                            : Theme.of(context).primaryColor,
                      ),
                    );
                  },
              ...
              CustomButton(
                ...
                onPressed: () {
                  if(widget.isStore) {
                    ...
                    Get.find<CategoryController>().sortItem();
