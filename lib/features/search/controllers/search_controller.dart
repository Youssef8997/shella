...
class SearchController extends GetxController implements GetxService {
  ...
  void setAllItemList(List<Item>? newItem) {
    if (newItem?.isNotEmpty==true) {
      _allItemList = newItem;
    } else {
...
List<int>categoryIds=[];
  ...
  if(!fromHome){
    ...
    if((_isStore && query!.isNotEmpty && query != _storeResultText) || (!_isStore && query!.isNotEmpty && (query != _itemResultText || fromHome))) {
      ...
      Response response = await searchServiceInterface.getSearchData(query, _isStore,categorysId:categoryIds,offset: offsetSearch );
      if (response.statusCode == 200) {
if(ItemModel.fromJson(response.body).items!.isEmpty){
  showCustomSnackBar("لا يوجد منتجات اضافيه");
  haveMoreItem=false;
}
        if (query.isEmpty) {
          if (_isStore) {
