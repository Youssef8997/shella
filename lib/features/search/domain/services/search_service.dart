...
class SearchService implements SearchServiceInterface {
  ...
  @override
  Future<Response> getSearchData(String? query, bool isStore,{List <int>? categorysId,offset}) async {
    return await searchRepositoryInterface.getList(query: query, isStore: isStore,categorysId: categorysId??[],offset: offset);
  }
  ...
  List<Item>? sortItemSearchList( List<Item>? allItemList, double upperValue, double lowerValue, int rating, bool veg, bool nonVeg, bool isAvailableItems, bool isDiscountedItems, int sortIndex) {
    ...
    if(sortIndex != -1) {
      if(sortIndex == 0) {
        searchItemList.sort((a, b) => (a.discountType=="amount"?a.price!-a.discount!:(a.price!-(a.price!*(a.discount!/100)))).compareTo(b.discountType=="amount"?b.price!-b.discount!:(b.price!-(b.price!*(b.discount!/100)))));
      }else {
        searchItemList.sort((a, b) => (a.discountType=="amount"?a.price!-a.discount!:(a.price!-(a.price!*(a.discount!/100)))).compareTo(b.discountType=="amount"?b.price!-b.discount!:(b.price!-(b.price!*(b.discount!/100)))));
