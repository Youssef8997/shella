...
class CheckoutController extends GetxController implements GetxService {
  ...
  List<XFile>? intsructionFile=[];
void setIntsructionFile(List<XFile>? file){
  intsructionFile=file;
  update();
}
  void toggleFirstOrder() {
    _isFirstOrder = !_isFirstOrder;
    update();
  }

  void toggleAddInvoice() {
    addToInvoice = !addToInvoice;
    update();
  }

  bool checkIFOrderFound(String invoiceId) {
    bool boolen = false;
    for (OrderModel order
        in Get.find<OrderController>().runningOrderModel!.collectedOrder!) {
      if (order.id == int.tryParse(invoiceId)) {
        boolen = true;
        break;
      } else {
        boolen = false;
    ...
    if (boolen == false) {
      print("asd");
      for (OrderModel order
          in Get.find<OrderController>().runningOrderModel!.orders!) {
        if (order.id == int.tryParse(invoiceId)) {
          boolen = true;
          break;
        } else {
  ...
  Future<void> initCheckoutData(int? storeId,{Store ?store}) async {
  if(_store?.id!=storeId){
    clearPrevData();
    _store = await Get.find<StoreController>()
        .getStoreDetails(store??Store(id: storeId), false);
  ...
  Future<String> placeOrder(
      ...
      {bool isOfflinePay = false,List<XFile>? orderAttachmentIntraction=const[],}) async {
    List<MultipartBody>? multiParts = [];
    for (XFile file in orderAttachment!) {
      multiParts.add(MultipartBody('order_attachment[]', file));
    }
    for (XFile file in orderAttachmentIntraction!) {
      multiParts.add(MultipartBody('order_instraction_attachemnet[]', file));
    }
    _isLoading = true;
    update();
    String orderID = '';
    ...
    if (isSuccess) {
      ...
      if (paymentMethodIndex == 2) {
        ...
        if (ResponsiveHelper.isDesktop(Get.context) &&
          ...
          await Get.toNamed(
            RouteHelper.getOrderDetailsRoute(int.tryParse(orderID),fromNotification: true),
