...
class OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  List <OrderModel>collectionOrders=[];
  ...
  void _startApiCall() {
    ...
    if(widget.orderModel?.isCollectionOrder==true){
      for(OrderModel? order in Get.find<OrderController>().runningOrderModel?.collectedOrder??[]){
for (int ordersId in widget.orderModel?.collectionsOrders??[]){
  if(ordersId==order!.id){
    collectionOrders.add(order);
  }
}
      }
    }
        ...
        body: SafeArea(
            child: GetBuilder<OrderController>(builder: (orderController) {
              ...
              if (orderController.orderDetails != null && order != null) {
                ...
                if(collectionOrders.isNotEmpty){
                  for(OrderModel?order in collectionOrders ) {
                    deliveryCharge=deliveryCharge+ double.parse(order!.deliveryCharge.toString());
                  }
                }
                ...
                if (prescriptionOrder) {
                  ...
                  in orderController.orderDetails!) {
                    ...
                    countOfStores=orderDetails.itemDetails!.storeId==widget.orderModel?.store!.id?countOfStores:countOfStores+1;
                    itemsPrice =
                        itemsPrice +
                ...
                if(collectionOrders.isNotEmpty) {
                  deliveryCharge=deliveryCharge+(countOfStores*Get.find<SplashController>().configModel!.priceForStop!);
                }
                  ...
                  Get.dialog(ConfirmationDialog(
                      ...
                      onYesPressed: () {
                        if ((((_maxCodOrderAmount != null &&
                          ...
                          showCustomSnackBar(
                              '${'you_cant_order_more_then'.tr} ${PriceConverter
                                  .convertPrice(
                                  _maxCodOrderAmount)} ${'in_cash_on_delivery'
                                  .tr}');
                        }
                      }));
                },
