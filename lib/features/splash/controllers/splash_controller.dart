...
class SplashController extends GetxController implements GetxService {
  ...
  List<ModuleModel>? _subModuleList;
  List<ModuleModel>? get moduleList => _moduleList;
  List<ModuleModel>? get subModuleList => _subModuleList;
  ...
  Future<void> getSubModules({Map<String, String>? headers,int?id}) async {
    _moduleIndex = 0;

    List<ModuleModel>? moduleList =
        await splashServiceInterface.getModules(headers: headers,id: id);
    if (moduleList != null) {
      _subModuleList = [];
      _subModuleList!.addAll(moduleList);
    }
    update();
  }
  ...
  void switchSubModule(int index, bool fromPhone) async {
      await Get.find<SplashController>().setModule(_subModuleList![index]);
