...
class SplashRepository implements SplashRepositoryInterface {
  ...
  @override
  Future<List<ModuleModel>?> getModules({Map<String, String>? headers,int?id}) async {
    List<ModuleModel>? moduleList;
    print("${id!=null?"${AppConstants.subModuleUri}?module_id=$id"
        :AppConstants.moduleUri} try to know");
    Response response = await apiClient.getData(id!=null?"${AppConstants.subModuleUri}?module_id=$id":AppConstants.moduleUri, headers: headers);
    if (response.statusCode == 200) {
      moduleList = [];
