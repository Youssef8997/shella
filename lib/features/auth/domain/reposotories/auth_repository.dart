...
class AuthRepository implements AuthRepositoryInterface {
  ...
  @override
  Future<List<PackagesModel>?> getPackage() async {
    Response response = await apiClient.getData(
      AppConstants.subscriptionsUri,
    );
    if (response.statusCode == 200) {
      return List.generate(response.body.length, (index) {
        return PackagesModel.fromJson(response.body[index]);
      });
    } else {
      return null;
    }
  ...
  @override
  Future<ResponseModel> subscribePacakages(int id) async {
    ResponseModel responseModel;

    Response response = await apiClient.getData(
      "${AppConstants.subscribeUri}/$id",
    );
    if (response.statusCode == 200) {
      await saveSharedPrefGuestId(response.body['guest_id'].toString());
