...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<Response> _getFeaturedStoreList() async {
    return await apiClient.getData(
      '${AppConstants.storeUri}/all?featured=1&offset=1&limit=5',
  ...
  Future<ItemModel?> getStoreItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.storeItemUri}?store_id=$storeID&category_id=$categoryID&offset=$offset&limit=5&type=$type');
  ...
  Future<RecommendedItemModel?> _getStoreRecommendedItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.storeRecommendedItemUri}?store_id=$storeId&offset=1&limit=5');
    if (response.statusCode == 200) {
  ...
  @override
  Future<List<Item>?> getRelatedProducts(String itemID) async{
    List<Item>relatedProducts=[];
    Response response=await apiClient.getData("${AppConstants.relatedItems}/$itemID");
    if(response.statusCode==200){

      relatedProducts=List.generate(response.body.length, (index){
        return Item.fromJson(response.body[index]);
      });
    }
