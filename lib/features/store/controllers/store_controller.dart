...
print("start");
    if (Get.find<CategoryController>().categoryList != null && _store != null) {
      ...
      await Get.find<CategoryController>().getCategoryList(true,allCategory: true);
      _categoryList = [];
      ...
      if(store?.is_subscription==false){
        for (var category in Get.find<CategoryController>().categoryList!) {
          if (_store!.categoryIdsss!.contains(category.id)) {
            _categoryList!.add(category);
          }else{
            print("${_store!.categoryIdsss!.contains(category.id)} no baby");
          }
        }
  ...
  Future<void> getStoreItemList(
    ...
    ItemModel? storeItemModel = await storeServiceInterface.getStoreItemList(
      storeID,
      offset,
      (_store != null && (_store?.is_subscription==false?true:_store!.categoryIds!.isNotEmpty) && _categoryIndex != 0)
          ? _categoryList![_categoryIndex].id
          : 0,
      type,
    );
    print(storeItemModel?.items?.length);
  ...
  Future<void> getStoreItemListSubCategory(
      int? storeID, int offset, String type, bool notify) async {
    if (offset == 1 || _storeItemModel == null) {
      _type = type;
      _storeItemModel = null;
    ...
    ItemModel? storeItemModel = await storeServiceInterface.getStoreItemList(
      ...
      (_store != null &&
              ...
              Get.find<CategoryController>().subCategoryIndex != 0)||_store!.is_subscription==false)
  ...
  void setCategoryIndex(int index, {bool itemSearching = false}) {
    ...
    if (itemSearching) {
      ...
      _storeItemModel = null;
      /* Get.find<CategoryController>().getSubCategoryList(index.toString());*/
  ...
  Future getRelatedItem(itemID)async{
    relatedProducts=await storeServiceInterface.getRelatedProducts(itemID.toString());
    update();
  }
}
