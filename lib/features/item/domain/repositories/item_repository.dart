...
class ItemRepository implements ItemRepositoryInterface {
  ...
  Future<List<Item>?> _getConditionsWiseItems(int id) async {
    ...
    Response response = await apiClient
        .getData('${AppConstants.conditionWiseItemUri}$id?limit=10&offset=1');
    if (response.statusCode == 200) {
  ...
  Future<ItemModel?> _getReviewedItemList(String type) async {
    ...
    if (response.statusCode == 200) {
      ...
      print("itemModel.items?.length ${itemModel.items?.length}");
    }
    return itemModel;
  }
  ...
  Future<ItemModel?> _getFeaturedCategoriesItemList() async {
    ...
    Response response = await apiClient.getData(
        '${AppConstants.featuredCategoriesItemsUri}?limit=10&offset=1',query:{
          "module":[]
    } );
    if (response.statusCode == 200) {
  ...
  Future<List<Item>?> _getRecommendedItemList(String type) async {
    ...
    Response response = await apiClient
        .getData('${AppConstants.recommendedItemsUri}$type&limit=10');
    if (response.statusCode == 200) {
      recommendedItemList = [];
  ...
  Future<List<Item>?> _getDiscountedItemList(String type) async {
    ...
    Response response = await apiClient.getData(
        '${AppConstants.discountedItemsUri}?type=$type&offset=1&limit=10');
    print('${AppConstants.discountedItemsUri}?type=$type&offset=1&limit=10');
    if (response.statusCode == 200) {
      discountedItemList = [];
