...
import 'package:sixam_mart/common/models/delivery_man_model.dart';
...
List<delevryManModels>_delevrys=[];
  WhyChooseModel? get whyChooseDetails => _whyChooseDetails;
  List<delevryManModels> get delevrys => _delevrys;
  ...
  Future<String> placeOrder(
    ...
    update();
    String orderID = '';
    Response response = await parcelServiceInterface.placeOrder(placeOrderBody,multiParts);
    _isLoading = false;
  ...
  Future getDeleveryMans(
    ) async {
    _isLoading = true;
    update();

    Response response = await parcelServiceInterface.getDeleveyMans();
    _isLoading = false;
    print("response.body${response.body}");
    if (response.statusCode == 200) {
      _delevrys=List.generate(response.body.length, (index)=>delevryManModels.fromJson(response.body[index]));
    } else {
