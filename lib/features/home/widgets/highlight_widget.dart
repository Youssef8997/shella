import 'package:carousel_slider/carousel_slider.dart' as t;
...
class _HighlightWidgetState extends State<HighlightWidget> {
  ...
  final t.CarouselSliderController _carouselController = t.CarouselSliderController();

  @override
  Widget build(BuildContext context) {
    return GetBuilder<HomeController>(builder: (advertisementController) {
        ...
        child: Stack(
          children: [
            ...
            CustomAssetImageWidget(
              Get.isDarkMode ? Images.highlihghtDarkBg : Images.highlightBg, width: context.width,
              fit: BoxFit.cover,
            ),
      ...
      if(videoPlayerController.value.duration == videoPlayerController.value.position){
        if(GetPlatform.isWeb){
          Future.delayed(const Duration(seconds: 4), () {
            Get.find<HomeController>().updateAutoPlayStatus(status: true, shouldUpdate: true);
          });
        }else{
          Get.find<HomeController>().updateAutoPlayStatus(status: true, shouldUpdate: true);
        }
